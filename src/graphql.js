...
/**
 * Formata dados do GraphQL para diferentes tipos de mídia (vídeo, imagem única, carrossel)
 */
    ...
    const mediaItems = data.edge_sidecar_to_children.edges.map(
      (edge, index) => {
        ...
        if (node.is_video) {
          return {
            type: "video",
            filename: getTimedFilename(`ig-carousel-${index}`, "mp4"),
          ...
          return {
            type: "image",
            filename: getTimedFilename(`ig-carousel-${index}`, "jpg"),
            width: itemDimensions.width.toString(),
            height: itemDimensions.height.toString(),
